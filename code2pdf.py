    ...
    def highlight_file(self, linenos=True, style='default'):
        ...
        try:
            if not os.path.isfile(self.input_file):
                print("NO FILE FOUND, SO CREATED", self.input_file)
...
def main(infile, outfile_postfix):
    file_prefix = infile[:-3]
    file_prefix = file_prefix #.replace('\\','/').replace("C:",'/mnt/c')
    
    py_filename = file_prefix+".py"
    pdf_filename = file_prefix+outfile_postfix+".pdf"

    pdf_file = get_output_file(py_filename, pdf_filename)
    pdf = Code2pdf(py_filename, pdf_file, "A5")
    pdf.init_print(linenos=False, style="fruity")
    return 0

if __name__ == "__main__":
    path = "C:\\Users\\niels\\OneDrive\\OneDriveDocs\\TA\\Numerical Methods\\"
    file = path+"Assignment2-2020.py"
    sys.exit(main(file,"_pycode"))
